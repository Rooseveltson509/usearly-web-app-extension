        ...
        <div className="login-popup">
          ...
          <form className='login-form' onSubmit={handleLogin} onClick={(e) => e.stopPropagation()}>
            ...
            <input
                ...
                id="remember"
                checked={rememberMe}
                onChange={() => setRememberMe(!rememberMe)} // Gérer l'état "Se souvenir de moi"
              />
          ...
          <button onClick={onClose} className="close-button-form" disabled={isSubmitting}>Fermer</button>
        </div>
      </div>
      {/* Flash message */}
      {flashMessage && (
        <>
          {console.log('FlashMessage prêt à être affiché :', flashMessage)}
          {console.log('FlashMessage prêt à être affiché dans le DOM :', flashMessage)}
          <FlashMessage
